...
async function getVideoNoWM(id: string): Promise<string> {
  return await (
    await (await fetch(`https://api16-normal-c-useast1a.tiktokv.com/aweme/v1/feed/?aweme_id=${id}`, { method: "GET", headers: headers })).json()
  ).aweme_list[0].video.play_addr.url_list[0];
}

async function getVideoId(url: string): Promise<string> {
  const matches = (await fetch(url, { method: "GET", headers: headersWm })).url.match(videoIdRegex);
