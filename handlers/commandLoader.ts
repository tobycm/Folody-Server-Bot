...
export default async function loadCommands(folody: Folody) {
  async function loadCommand(root: string, item: string): Promise<any> {
    ...
    const command = (await import(`.${root}${item}`)).default as MessageCommand | SlashCommand;
    if (command instanceof SlashCommand) return folody.slashCommands.set(command.data.name, command);

    folody.messageCommands.set(command.name, command);
    command.aliases?.forEach((alias) => folody.messageCommands.set(alias, command));
  }
